...
export const createProfileSchema = object({
  ...
  components: array()
    .of(string().required().oneOf([...ALL_SERVICES]))
    .notRequired(),
  feed_owner: string()
    .notRequired()
    .matches(ETH_ADDRESS_RE, 'feed_owner must be a 0x-prefixed Ethereum address'),
  feed_topic: string()
    .notRequired()
    .matches(FEED_TOPIC_RE, 'feed_topic must be printable ASCII, max 128 chars'),
  private_key: string()
    .notRequired()
