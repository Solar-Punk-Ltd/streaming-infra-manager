  ...
  implements EngineConfigOperationRepository
{
  ...
  private async locateRecovery(request: PreparedRecoveryDeploy): Promise<RecoveryLocator | null> {
    ...
    if (!operation || !profile || !recoveryOwnerMatches(request, profile, operation)) return null;
    ...
    return { operation, profile, descriptor, versionIds,
      problem: recoveryReferenceProblem(profile, operation, descriptor, references) };
  }
...
function recoveryOwnerMatches(input: PreparedRecoveryDeploy, profile: RecoveryProfile, row: OperationRow): boolean {
  ...
  return isDeepStrictEqual(rolloutProfileIdentity(profile), rolloutProfileIdentity(input.profile)) &&
    ...
    AUTOMATIC_RECOVERY_STATES.includes(row.state);
}
...
function recoveryOperationIdentity(row: OperationRow) {
  return { id: row.id, name: row.profile_name, instance: row.profile_instance_id, engine: row.engine, kind: row.kind,
    ...
    reference: row.recovery_reference_id, job: row.deployment_job_reference_id };
}
...
function recoveryReferenceProblem(
  profile: RecoveryProfile, row: OperationRow, descriptor: RolloutRecoveryDescriptor | null, references: readonly RecoveryReference[],
): string | null {
  if (descriptor?.kind !== 'immutable-build') return UNVERIFIED_RECOVERY;
  if (profile.stack_version_id !== descriptor.version.id || row.deployment_job_reference_id === null ||
  ...
  if (!sameSource(hold) || hold.holder_kind !== 'operation' || hold.holder_id !== String(row.id) || hold.resolved_at !== null ||
      !sameSource(job) || job.holder_kind !== 'job' || job.holder_id !== profile.name) return CHANGED_RECOVERY;
  return null;
}
