...
function harness(t: TestContext, options: OwnedTransferPreparationOptions = {}, intercept?: SyntheticBeeHandler) {
  ...
  const preparation = ChequebookTransferPreparation.fromOwnedTarget(async (name, instance) => {
    ...
    acquisitions++; signals.push(signal); seenProofs.push(proof);
    const acquired = await acquireDockerBeeStream(docker.transport, proof, budgets, image => image === syntheticImageId, signal);
  ...
  return { docker, target, preparation, repository, submission, signals, seenProofs, reader, counts: () => ({ captures, acquisitions }),
...
describe('owned Docker/Bee transfer preparation composition', { timeout: 5000 }, () => {
  it('keeps the successful preparation lease alive through preflight, claim and one actual framed Bee POST', async t => {
    const h = harness(t);
    ...
    // This fixture owns one connection. A separate fixture exercises complete submission below.
    assert.equal(journalProof, undefined);
  });
  ...
  it('freezes capture before acquisition and keeps the public proof immutable without altering its baseline', async t => {
    ...
    const result = await new ChequebookSubmission(h.repository, async () => prepared).submit(transferIntent());
