...
async function refuses(feed: (raw: SyntheticTransport, stream: Duplex) => void, options = bounds) {
  const raw = new SyntheticTransport();
  const stream = createDockerExecDuplex(raw, options);
  const outcome = finished(stream);
  stream.resume();
  feed(raw, stream);
  await assert.rejects(outcome, error => error instanceof DockerExecStreamError && error.cause === undefined && !error.message.includes('sensitive'));
...
describe('strict binary Docker exec duplex', { timeout: 5000 }, () => {
  ...
  it('enforces its total lifetime without traffic', () => refuses(() => {}, { ...bounds, totalTimeoutMs: 15 }));
