...
describe('approval invalidation history', () => {
  it('invalidates approval when a captured legacy refresh changes the commit', async () => {
    ...
    const unchanged = (await repository.captureLegacyMetadata())!;
    assert.equal(await repository.refreshLegacyMetadata(unchanged, { commitSha: COMMIT, contract: CONTRACT }), true);
    assert.equal((await repository.findById(id))?.tested, true);
    const changed = (await repository.captureLegacyMetadata())!;
    assert.equal(await repository.refreshLegacyMetadata(changed, { commitSha: NEXT_COMMIT, contract: CONTRACT }), true);
    const refreshed = (await repository.findById(id))!;
    assert.equal(refreshed.tested, false);
    assert.ok(refreshed.testedInvalidatedAt instanceof Date);
