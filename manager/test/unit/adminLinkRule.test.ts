 ...
 * by the version. And a token the deployment stores goes only to the origin
 * of the address it was stored for: a save and a deploy that would send it
 * elsewhere are refused, and Test connection asks nothing there.
 *
 * Unit test, no database, with the deploy script faked, through the real
 * routes, services and orchestrator over in-memory rows. `pnpm test` in
 * manager/.
 */
