...
import { PROFILE_COLUMNS } from '../../src/domain/profileSql.js';
...
describe('version removal before files disappear in isolated PostgreSQL', {
  ...
  async function insertDeployingProfile(name: string): Promise<Profile> {
    return (await pool.query<Profile>(
      `INSERT INTO profiles (name, kind, port_slot, status, stack_version_id)
       VALUES ($1, 'viewer', 1, 'DEPLOYING', $2) RETURNING ${PROFILE_COLUMNS}`, [name, selected.id],
    )).rows[0]!;
  }
  ...
  it('a reference-only INSERT first makes removal wait and then refuse before cleanup', async () => {
    ...
    try {
      ...
      await writer.query(
        "INSERT INTO build_references (version_id, build_id, holder_kind, holder_id, services) VALUES ($1,$2,'job','historical-owner',ARRAY['srs'])", [selected.id, BUILD],
      );
      ...
      assert.equal(called, false);
      assert.equal((await pool.query('SELECT COUNT(*)::int AS count FROM profiles')).rows[0].count, 0);
      await intact();
    } finally { await writer.query('ROLLBACK'); writer.release(); await removal?.catch(() => {}); }
  });

  it('removal first makes a later reference-only INSERT wait and refuse its deleted foreign key', async () => {
    const entered = signal(); const release = signal();
    ...
    try {
      ...
      assert.equal(await versions.findById(selected.id), null);
      assert.equal((await pool.query('SELECT COUNT(*)::int AS count FROM build_references')).rows[0].count, 0);
    } finally { release.resolve(); await writer.query('ROLLBACK'); writer.release(); await removal.catch(() => {}); await registration?.catch(() => {}); }
  });

  it('an assigned ledger owner first makes removal wait and refuse with its job retained', async () => {
    const entered = signal(); const release = signal();
    const profile = await insertDeployingProfile('synthetic-owner');
    ...
    const registration = registering.describe(profile.name, selected, ['srs'], deployOwnerOf(profile));
    let removal: Promise<unknown> | undefined;
    let called = false;
    try {
      await entered.promise;
  ...
  it('removal after detachment makes a stale ledger describe wait and refuse the deleted version', async () => {
    const profile = await insertDeployingProfile('synthetic-owner');
    await pool.query('UPDATE profiles SET stack_version_id = 1 WHERE name = $1', [profile.name]);
    const entered = signal(); const release = signal();
    const removal = versions.removeGuarded(selected, async () => { entered.resolve(); await release.promise; });
    let registration: Promise<unknown> | undefined;
    try {
      await entered.promise;
      registration = ledger.describe(profile.name, selected, ['srs'], deployOwnerOf(profile));
      await assertBlocked(); release.resolve(); await removal;
