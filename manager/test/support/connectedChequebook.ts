...
export async function startConnectedChequebook(options: ConnectedChequebookOptions): Promise<ConnectedChequebookBackend> {
  ...
  try {
    ...
    teardown.push(() => admin.end());
    await admin.query(`CREATE SCHEMA ${schema}`);
    teardown.push(async () => { await admin.query(`DROP SCHEMA IF EXISTS ${schema} CASCADE`); });
    const pool = new pg.Pool({ ...connection, max: 20, options: `-c search_path=${schema}` });
    teardown.push(() => pool.end());
    ...
    return {
      ...
      async close() {
        ...
        closed = true;
        const failures: unknown[] = [];
        try { await service.shutdown(); } catch (error) { failures.push(error); }
        failures.push(...await unwind());
