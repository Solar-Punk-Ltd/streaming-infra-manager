...
| PUT | `/profiles/:name/engine-settings` | `{ HLS_FRAGMENT?, HLS_SEGMENT_MAX?, HLS_WINDOW?, SRT_LATENCY?, ABR_*? }` for SRS, the three `HLS_*` keys for OME, the whole set, and `expectedInstanceId?` | 202 and the profile. For scripts: saves and recreates in one call, the engine container and the uploader with it when a key the uploader also reads changed. 409 `engine_settings_changed` when a save of the deployment's settings landed after it read them |
