...
The admin also has a labeled internal database network, `<admin-project>-fixture-db`. Its API joins both that network and the shared fixture network. Postgres joins only the database network, and the admin web service joins only the fixture network. An actual static Compose v5.5.1 merge with synthetic inputs confirmed these memberships after source correction 307d02b. This was a configuration check without a daemon operation. It does not prove runtime connectivity. The executor must journal this third network before guarded activation.

The uploader's two Compose volumes are `<profile>_srs-media` and `<profile>_uploader-state`. Their names are resolved during preflight, checked against the installed profile and included with the exact network ID in the durable transition identity. Stack adapter db0f381 checks the actual volume mounts, ownership labels and port bindings before a fixture receipt is eligible. Guard core 5628ad2b consumes that same contract. All guarded containers receive fixed CPU, memory and PID bounds, with runtime inspection required before media starts. Source corrections are reviewed through admin `ceae1ea9`, manager `3cf79ff1` and stack `e8bc3727`. Their fake-Docker regressions passed. Actual resource readings remain pending.

Bootstrap order is private chain and Bee readiness, guarded admin activation with managed ingest disabled, guarded manager activation, and real authenticated manager profile creation with the uploader held back. The returned profile UUID becomes the assignment in a second guarded admin activation. Viewer and uploader activation follow. The capable admin keeps enrollment unavailable until the latest four real receipts and fresh matching uploader capability observation are present. The installed guard submits receipts through the fixture admin's loopback endpoint. A lost reply remains an unresolved exact attempt. It is never repaired by fabricating a receipt or adopting objects by name alone. The typed two-stage plan is committed at `af760d21`. Concrete execution is still being wired.

The fixture guard installs only beneath `/home/solarpunk/srs-continuation-tests-20260920/<fixtureId>/guard`. Its parent must already be the canonical, non-symlink fixture directory. Missing build output refuses before creating that guard directory. The shared host installation under `.local` is unchanged. Source installer and immutable container-binding changes are reviewed through `b157a761`.
