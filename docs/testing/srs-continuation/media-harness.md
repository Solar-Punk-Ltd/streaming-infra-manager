...
The uploader's two Compose volumes are `<profile>_srs-media` and `<profile>_uploader-state`. Their names are resolved during preflight, checked against the installed profile and included with the exact network ID in the durable transition identity. Stack adapter db0f381 checks the actual volume mounts, ownership labels and port bindings before a fixture receipt is eligible. Guard core 5628ad2b consumes that same contract. All guarded containers must receive the same fixed CPU, memory and PID bounds recorded by the harness, and runtime inspection must confirm them before media starts. That resource-limit integration remains in progress.
