...
Status, 2026-09-08: this page describes the agreed main-v2 remediation. Version selection for new deployments already exists in the reviewed baseline `d046ebf`. The remediation branches add immutable builds, durable admission, port reservations, build-specific Tested approval and responsive version cards. They remain local until Levi integrates them. This document does not establish what is deployed on a host.
