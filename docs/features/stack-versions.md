...
The copy's files are the build's own files, hard linked rather than written again, the same way one build is cloned from another. A published build is never written to again, and the files a deployment writes into its copy are ones the build does not have, so the two can share the bytes of everything else. A copy of the real stack tree therefore costs the disk a few kilobytes of directories rather than another 432 MB, and the wait before a deploy's first script is the reading of the tree rather than a write of every byte in it. A file the filesystem will not link, because the versions root sits on another volume from the builds, gets its bytes copied instead, so such a host still deploys.

The copy is exact either way: its whole tree is inventoried and its digest compared to the source, and the source's per-path stamps are compared before the links are made and again after, so a build that moves under the copy is refused rather than run. The directories are owner only. Nothing in the current stack bind-mounts the tree into a container, so no engine uid needs to reach it, and a version that keeps no immutable builds gets no copy and runs from its flat tree as it always did.
