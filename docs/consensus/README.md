...
Where the work stands: `feat/ai-remediation` is merged into `main-v2`, and it was deployed to the live test host on 2026-09-11 along with its one-way migration and a signed-in pass. What that session found is the dated section at the end of `../handover/main-v2-remediation.md`. Everything in this directory is therefore a record of finished work rather than an instruction, and each file now says so on its own first lines. The rows added on 2026-09-17 under `issues/` are the exception, open work rather than record: T23's host measurement, T25's stack half until it is merged and pinned, and T27 pending the owner's wording. Read a brief for what was asked and a fixes file for what the reviewers measured, and treat the branches, worktrees, commits and line numbers inside them as the state at the time of writing.
