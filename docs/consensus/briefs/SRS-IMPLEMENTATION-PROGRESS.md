...
| R04 | Durable queue/admission checkpoints, closure retry and spool refusal are implemented. 6aeb1e79 binds the expected ladder. 4998c315 releases provisional sources. dd17cb77 and 68260af restore source/rung bindings in a fresh real router. 39ace82 persists a rung journal before its first media and prevents legacy recovery of a reserved corrupt managed record. The latest focused set passed 69 checks | Exclusive state writer, actual format checks, complete finalization/preparation, cross-admin-run A+B+C continuation and real process-crash evidence |
...
| R08 | Per-component installed guard and receipt protocol is frozen in the release brief. Admin 62515b3 adds bounded fresh uploader capabilities and immutable expected-rendition responses. f92b9a2 adds receipt storage and current-admin readiness evidence, with 5 focused PostgreSQL tests and 48 schema/file checks passing. Manager dabfe368 adds the external guard core, with 16 focused checks passing | Actual supported adapters and enrollment integration, legacy adoption, incompatible-release refusal through every real entry point and assembled candidate checks |
| R09 | Early engine probes and focused source/database tests exist. Removed Bee dev-mode assumption is corrected in the media-harness record, with matching pinned 2.8.2 private-chain image candidates identified | Complete isolated private-chain media harness and executed V01 to V22 inventory |
