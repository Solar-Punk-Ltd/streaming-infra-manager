...
| R04 | 1cac7de restores managed queues and same-run history in fresh uploader objects. c8bb423 retries closure persistence. 5dd6dd8 refuses callback acknowledgement on spool failure. 6aeb1e79 binds admission to an existing checkpoint and the frozen expected ladder, with 114 focused checks passing. 4998c315 releases a disconnected provisional source, with 24 focused checks passing | Fresh real-router startup integration, exclusive state writer, actual format checks, complete finalization, cross-admin-run A+B+C continuation and real process-crash evidence |
| R05 | Owner/internal operation routes and monotonic allocation exist. Nine database races passed. Full admin standard and database checks passed at e805886. 51c2c75 adds polling freshness. 0cf92e8 adds truthful continuation eligibility, close reasons and countdown, with 22 focused UI checks and database projection checks passing | Response-transit countdown adjustment, runtime preparation integration and later exact-candidate checks |
