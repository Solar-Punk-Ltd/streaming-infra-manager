...
| R04 | Durable queue/admission checkpoints, closure retry and spool refusal are implemented. 6aeb1e79 binds the expected ladder. 4998c315 releases provisional sources. dd17cb77 and 68260af restore source/rung bindings in a fresh real router, with 62 focused checks passing | Zero-media rung and corrupt-admission recovery variants, exclusive state writer, actual format checks, complete finalization/preparation, cross-admin-run A+B+C continuation and real process-crash evidence |
...
| R08 | Per-component installed guard and receipt protocol is frozen in the release brief. Admin 62515b3 adds bounded fresh uploader capabilities and immutable expected-rendition responses | Guard and enrollment integration, legacy adoption, incompatible-release refusal proof and assembled candidate checks |
