...
The deployment mode decision must remain exclusive until its chosen legacy operation finishes. A read of `legacy` followed by an unlocked standalone deploy is insufficient because activation can occur between them. The bounded implementation uses a shared bootstrap lease outside the optional guard root to exclude first installation, and the installed transition lock to exclude activation on an installed legacy host. Only confirmed completion releases an exact owned lease. Ambiguous remote completion preserves it for operator recovery. These source changes are still under implementation and do not authorize a host installation.
