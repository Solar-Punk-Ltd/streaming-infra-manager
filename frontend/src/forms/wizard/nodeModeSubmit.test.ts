...
describe('the chain endpoint on the wizard create body', () => {
  ...
  it('is absent where the deployment runs no node to reach a chain from', async (t) => {
    ...
    assert.equal('rpc_endpoint_source' in uploader.body, false);
    assert.equal('rpc_endpoint_source' in external.body, false);
    assert.equal('rpc_endpoint_source' in player.body, false);
  });
