...
for ((index = 1; index <= $#; index += 2)); do
    name="${!index}"
    case "$name" in
        --manager-mode | --manager-project-name | --manager-postgres-volume-name | --manager-postgres-port | --manager-web-port | --admin-project-name | --admin-postgres-volume-name | --admin-web-port) ;;
        *)
