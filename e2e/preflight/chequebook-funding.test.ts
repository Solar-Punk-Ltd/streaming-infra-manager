...
import { chequebookBalance, depositToChequebook, makeHost } from '../harness/host.js';
...
describe('preflight — bee-uploader chequebook is funded for bandwidth', () => {
  ...
  it(`holds at least ${MIN_CHEQUEBOOK_BZZ} BZZ (tops up from the node wallet if short)`, async () => {
    ...
    console.log(
      `  ${beeUploader} chequebook: ${plurToBzz(initialPlur)} BZZ total (need ≥ ${MIN_CHEQUEBOOK_BZZ})`,
    );
    ...
    await waitFor(
      async () => BigInt((await chequebookBalance(host, cfg)).totalBalance) >= MIN_CHEQUEBOOK_PLUR,
      {
