...
describe('service — viewer catalog via gateway reflects live→VOD', () => {
  ...
  it('surfaces a new live entry, then flips it to VOD on a clean stop', async () => {
    ...
    await waitFor(
      async () => (await safeFetch()).find((e) => e.topic === ourTopic)?.state === 'vod',
      { timeoutMs: VOD_WAIT_MS, intervalMs: 3_000, label: 'our catalog entry flips to VOD after the broadcaster stops' },
    );

    const finalEntry = (await safeFetch()).find((e) => e.topic === ourTopic);
    assert.equal(finalEntry?.state, 'vod', 'the entry must end as VOD');
    assert.ok((finalEntry?.duration ?? 0) > 0, `a VOD entry must carry a positive duration; got ${finalEntry?.duration}`);
