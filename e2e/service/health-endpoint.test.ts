...
describe('service — /health reflects the stream lifecycle', () => {
  ...
  it('reports an active healthy stream, then returns to idle after a clean stop', async () => {
    await waitFor(
      async () => parseUploaderLog(await host.logsSince(uploader, startedAt)).uploadedSegments.length >= WARMUP_SEGMENTS,
      { timeoutMs: SEGMENT_WAIT_MS, intervalMs: 2_000, label: `warmup: ${WARMUP_SEGMENTS} segments before checking /health` },
    );
